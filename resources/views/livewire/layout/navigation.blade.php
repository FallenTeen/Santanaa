        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @if(Auth::user()->role == 1)
                        ...
                        <x-nav-link :href="route('admin.viewhotel')" :active="request()->routeIs('admin.viewhotel')">
                            {{ __('Manage Hotel') }}
                        </x-nav-link>
                        <x-nav-link :href="route('admin.viewReport')" :active="request()->routeIs('admin.viewReport')">
                            {{ __('Laporan Pelanggan') }}
                        </x-nav-link>
                        <x-nav-link :href="route('admin.viewkaryawan')" :active="request()->routeIs('admin.viewkaryawan')">
                            {{ __('Manage Karyawan') }}
                        </x-nav-link>
                    ...
                    @elseif(Auth::user()->role == 2)
                        ...
                        <x-nav-link :href="route('karyawan.viewreport')" :active="request()->routeIs('karyawan.viewreport')">
                            {{ __('Lihat Report') }}
                        </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="flex items-center text-sm font-medium text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if(Auth::user()->role == 1)
                ...
                <x-responsive-nav-link :href="route('admin.viewkaryawan')" :active="request()->routeIs('admin.viewkaryawan')">
                    {{ __('View Karyawan') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('admin.viewhotel')" :active="request()->routeIs('admin.viewhotel')">
                    {{ __('View Hotel') }}
                </x-responsive-nav-link>
            @elseif(Auth::user()->role == 2)
                ...
                <x-responsive-nav-link :href="route('karyawan.viewreport')" :active="request()->routeIs('karyawan.viewreport')">
                    {{ __('Lihat Report') }}
                </x-responsive-nav-link>
            @elseif(Auth::user()->role == 3)
